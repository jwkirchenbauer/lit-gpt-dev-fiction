...
def condition_input_w_fiction_supervise_target(row, tokenizer, add_bos, add_eos):
    input_string = row["input_w_fiction"]
    joint_string = row["input_w_fiction"] + row["target"]
    input_tokens = tokenizer.encode(input_string, bos=add_bos, eos=False)
    joint_tokens = tokenizer.encode(joint_string, bos=add_bos, eos=add_eos)
    label_tokens = joint_tokens.clone()
    # mask the locations of the input tokens in the joint tokens
...
def condition_input_w_fictsheet_supervise_target(row, tokenizer, add_bos, add_eos):
    input_string = row["input_w_fictsheet"]
    joint_string = row["input_w_fictsheet"] + row["target"]
    input_tokens = tokenizer.encode(input_string, bos=add_bos, eos=False)
    joint_tokens = tokenizer.encode(joint_string, bos=add_bos, eos=add_eos)
    label_tokens = joint_tokens.clone()
    # mask the locations of the input tokens in the joint tokens
...
def condition_input_w_document_supervise_target(row, tokenizer, add_bos, add_eos):
    input_string = row["input_w_document"]
    joint_string = row["input_w_document"] + row["target"]
    input_tokens = tokenizer.encode(input_string, bos=add_bos, eos=False)
    joint_tokens = tokenizer.encode(joint_string, bos=add_bos, eos=add_eos)
    label_tokens = joint_tokens.clone()
    # mask the locations of the input tokens in the joint tokens
...
format_fn_registry = {
    ...
    "condition_input_w_fiction_supervise_target": condition_input_w_fiction_supervise_target,
    "condition_input_w_fictsheet_supervise_target": condition_input_w_fictsheet_supervise_target,
    "condition_input_w_document_supervise_target": condition_input_w_document_supervise_target,
...
def generic_collate_fn(
    ...
    inputs_batch = torch.full((len(batch), local_block_size), tokenizer.pad_id or 0, dtype=torch.int)
    labels_batch = torch.full((len(batch), local_block_size), tokenizer.pad_id or 0, dtype=torch.int)
