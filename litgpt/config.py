...
from typing import Any, Literal, Type, Union, Callable, Optional, List
...
from litgpt.utils import find_multiple, param_count_estimator
from litgpt.init import Init, init_normal  # For Jonas CE
...
class Config:
    ...
    init_strategy: str = "normal"  # For Jonas CE
    init_orthogonal: bool = False  # For Jonas CE
    ...
    # updated attn features for models like gemma-2-* and mistral
    norm_qk: bool = False
    post_attention_norm: bool = False
    post_mlp_norm: bool = False
    attn_bias: bool = False  # might not be used for those
    # take care of new way that attn weight is named
    ...
    _block_type: Literal["Block"] = "Block"
    ...
    _embed_spec: dict[str, Any] = field(default_factory=lambda: dict())
    _lm_head_spec: dict[str, Any] = field(default_factory=lambda: dict())
    _block_spec: list[dict[str, Any]] = field(
        default_factory=lambda: [dict(block_type="Block")]
    )  # if there is only one block type, use for all layers, else, specify for each layer override kwargs for each block, if unspecified, block will use fields in the model's cfg, else its default kwargs
    surrogate_config: bool = False
    use_rope: bool = True
    use_abacus: bool = False
    abacus_ids: Optional[List[int]] = None
    ...
    def __post_init__(self):
        ...
        if self.n_query_groups is not None:
            ...
            self.n_query_groups: int = self.n_head
        ...
        assert (len(self._block_spec) == self.n_layer) | (
            len(self._block_spec) == 1
        ), "Number of blocks must match the number of layers, or only one block type should be specified"

        # default behavior, all "Block"
        if len(self._block_spec) == 1:
            assert (
                self._block_spec[0].get("block_type") == "Block"
            ), "If only one block type, expecting it to be 'Block'"
            assert (
                not self.surrogate_config
            ), "If only one block type, and it is the default, not expected to be a surrogate config."
            # replicate the block spec for all layers, though this is actually unused by the non surrogate model file FIXME?
            self._block_spec = self._block_spec * self.n_layer
        elif set(bs["block_type"] for bs in self._block_spec) == {"Block"}:
            assert (
                not self.surrogate_config
            ), "If multiple block types manually spcified, but they're all 'Block', not expected to be a surrogate config."
            # no-op, all blocks are the same
        else:
            assert (
                self.surrogate_config
            ), "If multiple block types manually spcified, expected to be a surrogate config."

        # this is for Jonas CE
        self.init = Init(
            self.init_strategy,
            self.n_embd,
    ...
    def from_file(cls, path: Union[str, Path], **kwargs: Any) -> Self:
        with open(path, encoding="utf-8") as fp:
            # upstream now uses yaml by default, but certain dtypes don't parse reliably if file is actually json
            file_kwargs = json.load(fp) if path.suffix == ".json" else yaml.safe_load(fp)
    ...
    @property
    def mlp_class(self) -> Type[torch.nn.Module]:
        # `self.mlp_class_name` cannot be the type to keep the config serializable
        if self.surrogate_config:
            import litgpt.model_surrogates

            return getattr(litgpt.model_surrogates, self.mlp_class_name)
        else:
            import litgpt.model
    ...
    @property
    def norm_class(self) -> Union[Type[torch.nn.Module], Callable]:
        # `self.norm_class_name` cannot be the type to keep the config serializable
        if self.norm_class_name == "RMSNorm":
            from functools import partial

            if self.surrogate_config:
                from litgpt.model_surrogates import RMSNorm
            else:
    ...
    def construct_model(self, objective: dict[str, Any], gradient_checkpointing: bool) -> torch.nn.Module:
        # `self._mlp_class` cannot be the type to keep the config json serializable
        if self.surrogate_config:
            from litgpt.model_surrogates import GPT
        else:
            from litgpt.model import GPT

        return GPT(self, objective, gradient_checkpointing=gradient_checkpointing)
    ...
    def Block(self) -> Type[torch.nn.Module]:
        if self.surrogate_config:
            ...
            return getattr(litgpt.model_surrogates, self._block_type)
        else:
            import litgpt.model

            return getattr(litgpt.model, self._block_type)
...
llama_2 = [
    dict(
        ...
        vocab_size=32000,
        block_size=4096,  # to find it again
        n_head=8,
        n_query_groups=2,
        n_embd=512,
        padding_multiple=128,
...
llama_3 = [
    ...
    dict(
        name="Meta-Llama-3-8B-Base-w-Instruct-tokens",
        hf_config=dict(org="tomg-group-umd", name="Meta-Llama-3-8B-Base-w-Instruct-tokens"),
        block_size=8192,
        vocab_size=128256,
        padding_multiple=64,
        n_layer=32,
    ...
    dict(
        name="Llama-3.1-8B-Base-w-Instruct-tokens",
        hf_config=dict(org="meta-llama", name="Meta-Llama-3.1-8B{}"),
        block_size=8192,
        vocab_size=128000,
        padded_vocab_size=128256,
...
scaling_laws_gemma = [
    dict(
        name="PyGemma-2-2b-jonas",  # For Jonas CE
        hf_config=dict(org="google", name="gemma-2-2b"),
        scale_embeddings=True,
        vocab_size=50304,
...
pygemma_widths_and_depths = chain(
    # 50M
    product([256], [23, 27]),
    # 100M
    product([256], [71, 80]),
...
# assumptions
head_size = 128
get_n_heads = lambda width: width // head_size
get_n_query_groups = lambda width: width // head_size // 2
get_intermediate_size = lambda width: width * 4
base_config = list(filter(lambda x: x["name"] == "PyGemma-base", pygemma))[0]
for width, depth in pygemma_widths_and_depths:
    copy = deepcopy(base_config)
    copy["name"] = f"PyGemma-{width}x{depth}"
    copy["n_head"] = get_n_heads(width)
    copy["head_size"] = head_size
    copy["n_query_groups"] = get_n_query_groups(width)
    copy["n_embd"] = width
    copy["n_layer"] = depth
    copy["intermediate_size"] = get_intermediate_size(width)
    pygemma.append(copy)

configs.extend(pygemma)

# print(f"PyGemma configs: {len(pygemma)}")
# for c in pygemma:
#     print(c)
#     pcount = param_count_estimator(
#         width=c["n_embd"],
#         depth=c["n_layer"],
#         vocab_size=c["vocab_size"],
#         n_head=c["n_head"],
#         head_size=c["head_size"],
#         n_query_groups=c["n_query_groups"],
...
tiny_llama = [
    ...
    dict(
        name="tiny-llama-1.1b-Instruct",
        hf_config=dict(org="tomg-group-umd", name="TinyLlama-1.1B-Instruct"),
        block_size=2048,
        vocab_size=32003,
    ...
    dict(
        name="tiny-llama-1.1b-Instruct-padded",
        hf_config=dict(org="tomg-group-umd", name="TinyLlama-1.1B-Instruct"),
        block_size=2048,
        vocab_size=32003,
    ...
    dict(
        name="tiny-llama-1.1b-Instruct-32005",
        hf_config=dict(org="tomg-group-umd", name="TinyLlama-1.1B-Instruct-32005"),
        block_size=2048,
        vocab_size=32005,
        padded_vocab_size=32064,
        padding_multiple=64,
    ...
    dict(
        name="tiny-llama-1.1b-Instruct-padded-4096",
        hf_config=dict(org="tomg-group-umd", name="TinyLlama-1.1B-Instruct-padded-4096"),
        block_size=4096,
        vocab_size=32003,
        padded_vocab_size=32064,
        padding_multiple=64,
    ...
    dict(
        name="tiny-llama-3-1_3b",
        hf_config=dict(org="tomg-group-umd", name="tiny-llama-3-1_3b"),
        block_size=8192,
        vocab_size=128256,
        padding_multiple=64,
        n_embd=4096,
        # head_size= this _must_ be n_embd / n_head according to hf llama modeling file, but also <= 256 according to flash attn
        n_head=16,  # which means for n_emb 4096, min n_heads is 16
        n_query_groups=4,
        n_layer=2,
        rotary_percentage=1.0,
        parallel_residual=False,
    ...
    dict(
        name="tiny-llama-3-mlp-squeeze-block{}",
        hf_config=dict(org="tomg-group-umd", name="tiny-llama-3-mlp-squeeze-block{}"),
        block_size=8192,
        vocab_size=128256,
        padding_multiple=64,
        n_embd=2048,
        # head_size= this _must_ be n_embd / n_head according to hf llama modeling file, but also <= 256 according to flash attn
        n_head=32,  # which means for n_emb 4096, min n_heads is 16
        n_query_groups=4,
        n_layer=4,
        rotary_percentage=1.0,
        parallel_residual=False,
        ...
        intermediate_size=5632,
        rope_base=500000,
        surrogate_config=True,
        _embed_spec=dict(n_embd=4096),
        _lm_head_spec=dict(n_embd=2048),
        _block_spec=[
            dict(
                block_type="SqueezeBlock",
                n_embd=4096,
                head_size=128,  # locally larger head size than n_embd / n_head for rest of model
                mlp_class_name="AsymmetricLLaMAMLP",
                n_embd_mlp_in=4096,
                intermediate_size=8192,
                n_embd_mlp_out=2048,
            )
        ...
        + [
            dict(block_type="Block"),
        ]
        * 3,
    ),
    dict(
        name="tiny-llama-1.4b-50k",  # a tinyllama model with diff shapes and pythia 50k vocab.
        hf_config=dict(org="TinyLlama", name="TinyLlama-1.1B{}"),
        block_size=2048,
        vocab_size=50304,
        padding_multiple=64,
...
for c in filter(lambda c: "squeeze-block" not in c["name"], tiny_llama):
...
# tinyllama3 mlp squeeze depth variants
for c in filter(lambda c: "squeeze-block" in c["name"], tiny_llama):
    for l, kind, hf_postfix in [(l, f"-{l}l", f"-{l}l") for l in range(4, 25, 4)]:
        copy = deepcopy(c)
        copy["name"] = c["name"].format(kind)
        copy["hf_config"]["name"] = c["hf_config"]["name"].format(hf_postfix)
        copy["n_layer"] = l
        copy["_block_spec"] = [copy["_block_spec"][0]] + [dict(block_type="Block")] * (l - 1)
        configs.append(copy)
