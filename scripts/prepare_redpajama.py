...
import litgpt.packed_dataset as packed_dataset
        ...
        if is_cc:
            ...
            with open(filepath, encoding="utf-8") as f:
                for row in tqdm(f):
                    ...
                    builder.add_array(np.array(text_ids, dtype=builder.dtype))
        with open(
            "/lustre/orion/csc569/scratch/njain17/new_workspace/lit-gpt-dev/tokenize_redpajama_filenames_processed.txt",
            "a",
        ) as f:
            print(name, file=f)
...
def prepare_full(
    ...
    chunk_size: int,
    match: str = "",
    limit_file_num: int = -1,
    files_per_chunk_org: int = 2,
    skip_cc: bool = False,
    only_cc: bool = False,
    subset_prefix: str = "",
) -> None:
