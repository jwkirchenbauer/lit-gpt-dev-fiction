...
def copy_weights_falcon(
    ...
    assert not resize_embeddings, "Resizing embeddings is not supported for Falcon models."
...
def copy_weights_gpt_neox(
    ...
    assert not resize_embeddings, "Resizing embeddings is not supported for GPT-NeoX models."
...
def copy_weights_llama(
    ...
    for name, param in lit_weights.items():
        ...
        if q2_codebase:
            name = name.replace("_original_module.", "")  # q2 version of compile artifact?
            if axonn_patch:
                name = name.replace("_forward_module.module.", "")  # q2 version of axonn artifact?
        else:
            name = name.replace("_orig_mod.", "")  # compile artifact?
            if axonn_patch:
                name = name.replace("module.", "")  # axonn artifact?
        ...
        if name.endswith(".attn.attn.weight"):
            ...
            qp, kp, vp = qkv_split(qkv, config, l)
            ...
            if resize_embeddings and (name.endswith("transformer.wte.weight") or name.endswith("lm_head.weight")):
                # resize embeddings
                print("Param size before:", param.size())
                new_size = (config.vocab_size, param.size(1))
                param = param[: config.vocab_size]
                print("Param size after:", param.size())
...
def copy_weights_gemma2(
    ...
    weight_map = {
        ...
        "transformer.h.{}.mlp.proj.weight": "model.layers.{l}.mlp.down_proj.weight",
        "transformer.h.{}.norm_1.weight": "model.layers.{l}.input_layernorm.weight",
        "transformer.h.{}.post_attention_norm.weight": "model.layers.{l}.post_attention_layernorm.weight",
        "transformer.h.{}.norm_2.weight": "model.layers.{l}.pre_feedforward_layernorm.weight",
        "transformer.h.{}.post_mlp_norm.weight": "model.layers.{l}.post_feedforward_layernorm.weight",
        "transformer.ln_f.weight": "model.norm.weight",
        "lm_head.weight": "lm_head.weight",
    }
...
def copy_weights_phi(
    ...
    assert not resize_embeddings, "Resizing embeddings is not supported for Phi models."
    weight_map = {
        "transformer.wte.weight": "model.embed_tokens.weight",
...
def qkv_split(
    param: Union[torch.Tensor, NotYetLoadedTensor], config: Config, layer: int
    ...
    for chunk in torch.chunk(param, config.n_query_groups):
        if config._block_spec is not None:
            curr_block_spec = config._block_spec[layer]
            if curr_block_spec.get("block_type") == "SqueezeBlock":
                assert (
                    curr_block_spec.get("head_size", None) is not None
                ), "Head size must be specified for SqueezeBlock conversion."
                local_head_size = curr_block_spec["head_size"]
            else:
                local_head_size = config.head_size
            split = torch.split(chunk, [local_head_size * q_per_kv, local_head_size, local_head_size])
        else:
...
def convert_lit_checkpoint(
    ...
    # if the padded vocab size is not the same as the model's vocab size, we need to resize the embeddings
    if config.vocab_size != config.padded_vocab_size:
        if not resize_embeddings:
            print(
                "Warning: if the padded vocab size is different from the model's vocab size, might want to resize the embeddings before exporting to hf."
            )
        if resize_embeddings:
            # We assume that the padded size is larger than the model's vocab size
            assert (
                config.vocab_size <= config.padded_vocab_size
            ), "The padded vocab size must be larger than the model's vocab size"

    output_path.parent.mkdir(parents=True, exist_ok=True)

    if "falcon" in config.name:
        copy_fn = partial(copy_weights_falcon, config.name)
    elif config.name.lower().startswith("gemma-2-"):  # limit the new logic to just gemma-2-* series
        copy_fn = partial(
            copy_weights_gemma2,
            config,
