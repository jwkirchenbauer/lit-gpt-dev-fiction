...
from litgpt.utils import CLI, incremental_save
...
def convert_checkpoint(checkpoint_file: Path, tokenizer_dir: Path, config_name: str, output_dir: Path) -> None:
    ...
    Args:
        checkpoint_file: Path to a checkpoint file scripts produced by the scripts in ``litgpt/pretrain/``.
        ...
        config_name: The name of the model loaded with the ``litgpt.Config``. The configuration will be saved as a
    ...
    with incremental_save(output_checkpoint_file) as saver:
        ...
        full_checkpoint = torch.load(str(checkpoint_file), mmap=True, weights_only=False)
