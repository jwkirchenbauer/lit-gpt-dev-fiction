...
from litgpt.config_dynamic import Config
...
def copy_weights_gpt_neox(
    ...
    weight_map = {
        ...
        "gpt_neox.layers.{}.attention.query_key_value.bias": "transformer.h.{}.attn.Wqkv.bias",
        "gpt_neox.layers.{}.attention.query_key_value.weight": "transformer.h.{}.attn.Wqkv.weight",
...
def copy_weights_falcon(
    ...
    weight_map = {
        ...
        "transformer.h.{}.self_attention.query_key_value.weight": "transformer.h.{}.attn.Wqkv.weight",
...
def copy_weights_hf_llama(
    ...
    fc_weights: Dict[int, List[Optional[NotYetLoadedTensor]]],
    state_dict: Dict[str, torch.Tensor],
    hf_weights: Dict[str, Union[torch.Tensor, NotYetLoadedTensor]],
    saver: Optional[incremental_save] = None,
    dtype: Optional[torch.dtype] = None,
    ...
    for name, param in hf_weights.items():
        if "model.layers" in name:
            ...
            fc = fc_weights.setdefault(l, [None, None])
            if "q_proj" in name:
                qkv[0] = param
            elif "k_proj" in name:
                qkv[1] = param
    ...
    for i, (q, k, v) in list(qkv_weights.items()):
        ...
        q_per_kv = config.num_key_value_heads
        qs = torch.split(q, config.head_size * q_per_kv)
        ks = torch.split(k, config.head_size)
        vs = torch.split(v, config.head_size)
        cycled = [t for group in zip(qs, ks, vs) for t in group]
        qkv = torch.cat(cycled)
        state_dict[f"transformer.h.{i}.attn.Wqkv.weight"] = qkv
        del qkv_weights[i]

    # convert gated block matrices into one
    for i, (fc1, fc2) in list(fc_weights.items()):
        if fc1 is None or fc2 is None:
            # split across different .bin files
            continue
        fc1 = load_param(fc1, f"layer {i} gate_proj", dtype)
        fc2 = load_param(fc2, f"layer {i} up_proj", dtype)
        fc = torch.cat([fc1, fc2], dim=0)
        state_dict[f"transformer.h.{i}.mlp.fc.weight"] = fc
        del fc_weights[i]
...
def copy_weights_phi(
    ...
    for i in list(qkv_weights):
        for weight_type in list(qkv_weights[i]):
            ...
            q_per_kv = config.num_attention_heads // config.n_query_groups
            qs = torch.split(q, config.head_size * q_per_kv)
            ks = torch.split(k, config.head_size)
            vs = torch.split(v, config.head_size)
            cycled = [t for group in zip(qs, ks, vs) for t in group]
            qkv = torch.cat(cycled)
            state_dict[f"transformer.h.{i}.attn.Wqkv.{weight_type}"] = qkv
...
def convert_hf_checkpoint(
    *,
    ...
    with open(checkpoint_dir / "lit_config_dynamic.json", "w") as json_config:
    ...
    if "falcon" in model_name:
        ...
        qkv_weights = {}
        fc_weights = {}
        copy_fn = partial(copy_weights_hf_llama, config, qkv_weights, fc_weights)
    elif "phi" in model_name:
        # holder to reconstitute the split q, k, v
        qkv_weights = {}
        fc_weights = {}
        copy_fn = partial(copy_weights_phi, config, qkv_weights, fc_weights)
    else:
    ...
    with incremental_save(checkpoint_dir / "lit_model_dynamic.pth") as saver:
